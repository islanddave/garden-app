// OPS-L081COLS-001 — static SELECT-column contract for daily-plan.
//
...
//
// daily-plan is the nightly Lambda that builds Today. It queries via node-postgres `.query()` rather than a
// sql`` tagged template, which is exactly why an extractor that only understood sql`` reported it as
// having no SQL at all — see the enrollment ratchet's note on that near-miss.
//
// The list below is EVIDENCE-DERIVED: the intersection of weather_daily's live prod columns with the
...
const AUDIT_TABLES = ['weather_daily'];
...
describe('daily-plan SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['weather_daily']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(8);
  });
});
