// OPS-L081COLS-001 — static SELECT-column contract for evidence-ingest.
//
...
//
// evidence-ingest writes the DrG evidence spine (29 of 35 columns referenced) — the widest single-table
// surface outside event_log.
//
// The list below is EVIDENCE-DERIVED: the intersection of evidence's live prod columns with the
...
describe('evidence-ingest SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['evidence']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(27);
  });
});
