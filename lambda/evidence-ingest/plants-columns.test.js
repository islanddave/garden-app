// OPS-SCHEMAAUDITJOIN-001 — the public.plants columns lambda/evidence-ingest reads.
//
// Both statements are authz-parents.js, the write-FK parent resolver that is byte-identical
// in seven directories (one sha256) because each Lambda is zipped from its OWN directory, so
// a `../authz-parents.js` import is not packaged and the handler 502s at module load. Phase 4
// groups by Path(handler).parent, so the copy in this directory needs its own contract. Both
// address the BASE table rather than the garden_node VIEW over it, which is where the
// ownership CHECK constraints live (authz-parents.js:35-38).
//
...
describe('OPS-SCHEMAAUDITJOIN-001 — lambda/evidence-ingest plants column contract', () => {
