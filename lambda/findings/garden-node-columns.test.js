// OPS-SCHEMAAUDITJOIN-001 — the public.garden_node columns lambda/findings reads.
//
// A single JOIN, aliased `p` — the same alias letter the BUG-SEEDDETAIL500-001 regression used
// when it selected `p.name` off this table. `name` does not exist on garden_node; `display_name`
// does, and the negative guard below pins that.
//
...
const AUDIT_COLUMNS = {
  garden_node: ['archived_at', 'container_id', 'deleted_at', 'display_name', 'id', 'status'],
};
...
describe('OPS-SCHEMAAUDITJOIN-001 — lambda/findings garden_node column contract', () => {
