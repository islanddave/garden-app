// OPS-L081COLS-001 — static SELECT-column contract for achievements.
//
...
//
// achievements drives the reward surface; a missing column here silently stops awards rather than erroring
// visibly, which is the hardest failure class to notice.
//
// The list below is EVIDENCE-DERIVED: the intersection of achievements's live prod columns with the
...
const AUDIT_TABLES = ['achievements'];
...
describe('achievements SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['achievements']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(9);
  });
});
