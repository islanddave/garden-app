// OPS-L081COLS-001 — static SELECT-column contract for shared-state.
//
...
//
// garden_shared_state backs the household shared-state sync.
//
// The list below is EVIDENCE-DERIVED: the intersection of garden_shared_state's live prod columns with the
...
const AUDIT_TABLES = ['garden_shared_state'];
...
describe('shared-state SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['garden_shared_state']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(5);
  });
});
