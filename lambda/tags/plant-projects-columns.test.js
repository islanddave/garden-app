// OPS-SCHEMAAUDITJOIN-001 — the public.plant_projects columns lambda/tags reads.
//
// One statement: entityExists('project') in index.js, the ownership gate a tag attach passes
// before it may name a container. It binds no alias, so its identifiers cannot be attributed
// automatically and are pinned by hand below. Note it selects a bare 1 rather than the row, so
// unlike the authz-parents.js copy of the same predicate it reads no `name`.
//
...
const AUDIT_COLUMNS = {
  plant_projects: ['created_by', 'deleted_at', 'id'],
};
...
const UNALIASED_ARMS = [
  {
    file: 'index.js',
    // entityExists('project'). authz-parents.js:48 records that its loadOwnedProject predicate
    // was lifted byte-for-byte from this line; this copy is the original and selects a bare 1,
    // so it touches id, deleted_at and created_by but not name.
    pin: /SELECT 1 FROM public\.plant_projects WHERE id = \$\{entityId\} AND deleted_at IS NULL AND created_by = ANY\(\$\{household\}\)/,
    columns: ['id', 'deleted_at', 'created_by'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/tags plant_projects column contract', () => {
