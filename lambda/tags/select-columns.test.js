// OPS-L081COLS-001 — static SELECT-column contract for tags.
//
...
//
// tag + entity_tag back the faceting surface. Prod entity_tag is nearly empty (2 rows), so a schema drift
// here would be invisible in behaviour and visible only to this audit.
//
// The list below is EVIDENCE-DERIVED: the intersection of tag's live prod columns with the
...
describe('tags SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['tag']);
  });
