// OPS-L081COLS-001 — static SELECT-column contract for storage-location.
//
...
//
// storage_location is the pantry's shelf vocabulary, referenced by preservation_log rows that outlive their
// plantings.
//
// The list below is EVIDENCE-DERIVED: the intersection of storage_location's live prod columns with the
...
const AUDIT_TABLES = ['storage_location'];
...
describe('storage-location SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['storage_location']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(4);
  });
});
