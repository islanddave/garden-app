// OPS-L081COLS-001 — static SELECT-column contract for members.
//
...
//
// members reads locations and inventory_items from a SIBLING module, not index.js. It was briefly and
// WRONGLY marked exempt on 2026-08-14 by a check that scanned only index.js for sql`` templates; this
// enrolment is the correction, and the ratchet's exemption test was widened in the same change.
//
// The list below is EVIDENCE-DERIVED: the intersection of locations's live prod columns with the
...
describe('members SELECT-column contract (L-081 Phase 1)', () => {
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(3);
  });
});
