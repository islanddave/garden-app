// OPS-L081COLS-001 — static SELECT-column contract for critter.
//
...
//
// critter_state carries the pest/beneficial tracking the critter quiet-hours logic reads.
//
// The list below is EVIDENCE-DERIVED: the intersection of critter_state's live prod columns with the
...
describe('critter SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['critter_state']);
  });
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(13);
  });
});
