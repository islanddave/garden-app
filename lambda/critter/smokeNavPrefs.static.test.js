// V5-NAVCUSTOM-001 — static guard on the staging smoke's block L (tests/smoke/run-smoke.sh), the
// write-then-read-back of more_pins and bar_layout through the critter Lambda.
//
// WHY A FILE-READING TEST. Block L runs only inside deploy-staging.yml, against the staging Lambda;
...
// here. So every body block L and cleanup() send is parsed out of the script and put through this
...
const blockStart = SMOKE.indexOf('# ── L) Per-person nav prefs')
...
const shellJson = (name) => {
  ...
  expect(m, `${name}='…' in block L`).not.toBeNull()
  return JSON.parse(m[1])
}

describe('smoke block L — the bodies it sends are ones this Lambda accepts', () => {
  it('SELF-TEST: block L and cleanup() were found, block L after Phase 2 and gated on the critter URL', () => {
