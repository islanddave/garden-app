// OPS-SCHEMAAUDITJOIN-001 — the public.plant_projects columns lambda/plants reads.
//
// Four statements. Three are authz-parents.js, the write-FK parent resolver that is
// byte-identical in seven directories (one sha256) because each Lambda is zipped from its OWN
// directory, so a `../authz-parents.js` import is not packaged and the handler 502s at module
// load. The fourth is merge.js's merge-group gate, which reads the same three columns through
// the same `pp` alias and additionally requires `pp.deleted_at IS NULL` — a merge must not
// adopt a planting whose container has been soft-deleted.
//
...
describe('OPS-SCHEMAAUDITJOIN-001 — lambda/plants plant_projects column contract', () => {
