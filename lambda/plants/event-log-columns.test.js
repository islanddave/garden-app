// OPS-SCHEMAAUDITJOIN-001 — the public.event_log columns lambda/plants reads.
//
// Twelve statements across four handlers, three of them in merge.js written with NO alias.
// merge.js is the planting-merge path, so these columns decide what a merge dedups, what it
// repoints and what its concurrency fingerprint compares — updated_at is reached ONLY by that
// fingerprint, and created_at and metadata ONLY by the dedup plan.
//
...
// than hardcoded, so a handler added here that JOINs event_log is covered the day it lands
...
// L-081 KEYED contract. Every column below verified present on public.event_log in live prod Neon on
// 2026-08-29 (28 columns), read through the read-only role.
...
const AUDIT_COLUMNS = {
  event_log: [
    'created_at', 'created_by', 'deleted_at', 'event_date', 'event_type', 'flagged_as_issue',
    'id', 'metadata', 'plant_id', 'project_id', 'updated_at'
  ],
};

const EVENT_LOG_COLUMNS = AUDIT_COLUMNS.event_log;
...
// group is OPTIONAL: an unaliased `FROM public.event_log WHERE ...` captures the next keyword,
// which NOT_AN_ALIAS rejects and UNALIASED_ARMS then has to account for by hand.
const bindings = (s) => [...s.matchAll(
  /\b(?:FROM|JOIN)\s+(?:public\.)?event_log\b(?!\s*\.)\s*(?:AS\s+)?([a-z_][a-z0-9_]*)?/gi,
...
// Scoped to statements that BIND event_log, so an `x.col` belonging to some other query in the
...
// Reads that name event_log with NO alias. Nothing can attribute their bare identifiers
...
const UNALIASED_ARMS = [
  {
    file: 'merge.js',
    // The dedup plan over the merge group's live events. Its ORDER BY (created_at, id) is
    // what makes the plan deterministic, so both columns are load-bearing beyond being selected.
    pin: /SELECT id, plant_id, event_type, event_date, created_at, metadata\s+FROM event_log\s+WHERE plant_id = ANY\(/,
    columns: [
      'id', 'plant_id', 'event_type', 'event_date', 'created_at', 'metadata', 'deleted_at'
    ],
  },
  {
    file: 'merge.js',
    // The repoint ledger row. `plant_id AS old_value` is what lets a merge be reversed;
    // the alias is part of the wire shape the repoint list renders, not decoration.
    pin: /SELECT id, plant_id AS old_value FROM event_log WHERE plant_id = ANY\(/,
    columns: ['id', 'plant_id'],
  },
  {
    file: 'merge.js',
    // readFingerprint's set-level concurrency guard. It is the ONLY event_log read of
    // updated_at anywhere in this directory, so this pin is the only thing holding that column.
    pin: /max\(updated_at\) AS max_updated_at\s+FROM event_log WHERE plant_id = ANY\(/,
    columns: ['updated_at', 'plant_id', 'deleted_at'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/plants event_log column contract', () => {
  it('finds the event_log statements, so the assertions below are not vacuous', () => {
  ...
  it('accounts for every unaliased event_log read', () => {
  ...
  it('references no column absent from the contract, and declares none it does not use', () => {
    ...
    expect(referenced).toEqual([...EVENT_LOG_COLUMNS].sort());
  });

  it('never reaches for a column that belongs to another table', () => {
    // None of these exist on event_log, and each is a live confusion: container_id and
    // display_name are garden_node's, the date column is event_date not event_at, and the two actor
    // columns are created_by and logged_by, never user_id.
    const NOT_ON_TABLE = ['container_id', 'display_name', 'name', 'event_at', 'user_id'];
    for (const col of NOT_ON_TABLE) {
      expect(EVENT_LOG_COLUMNS).not.toContain(col);
      for (const { file, sql } of STATEMENTS) {
        for (const a of aliasesOf(sql)) {
          expect(sql, `${file}: ${a}.${col} is not a event_log column`)
  ...
  it('exposes the contract in the shape scripts/dev-main-schema-audit.py can parse', () => {
    ...
    expect(pairs.map((m) => m[1])).toEqual(['event_log']);
    const cols = [...pairs[0][2].matchAll(/['"]([a-zA-Z_][a-zA-Z0-9_]*)['"]/g)].map((m) => m[1]);
    expect(cols).toEqual(EVENT_LOG_COLUMNS);
  });
});
