// OPS-SCHEMAAUDITJOIN-001 — the public.container columns lambda/ux-events reads.
//
// The third arm of the same admin capture-week UNION that carries this directory's only
// event_log and garden_node references. No alias, so the alias set below is empty and the contract
// rests entirely on the pin.
//
...
const AUDIT_COLUMNS = {
  container: ['archived_at', 'created_at'],
};
...
const UNALIASED_ARMS = [
  {
    file: 'index.js',
    // M2, captures-per-week. V4-ARCHIVEHIDE-001 (L7) added the archived_at filter to this
    // arm and to the garden_node one beside it, deliberately NOT to the event_log arm.
    pin: /UNION ALL SELECT created_at FROM public\.container WHERE archived_at IS NULL/,
    columns: ['created_at', 'archived_at'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/ux-events container column contract', () => {
