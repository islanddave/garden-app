// OPS-SCHEMAAUDITJOIN-001 — the public.garden_node columns lambda/ux-events reads.
//
// THE CASE AN ALIAS SCAN CANNOT SEE AT ALL. This directory's only garden_node reference is one
// arm of a three-way UNION inside the admin capture-events metric, written with no alias. The alias
// set below is therefore empty and every column in the contract comes from the pinned arm — if the
// pin ever stops matching, this file reds rather than passing on a contract it can no longer
// check.
//
...
const AUDIT_COLUMNS = {
  garden_node: ['archived_at', 'created_at'],
};
...
const UNALIASED_ARMS = [
  {
    file: 'index.js',
    // M2, captures-per-week. V4-ARCHIVEHIDE-001 (L7) added the archived_at filter; the arm
    // deliberately does NOT filter deleted_at, so the two lifecycle axes are asymmetric here on
    // purpose. Pinning the whole line keeps that asymmetry from being "tidied up" unnoticed.
    pin: /UNION ALL SELECT created_at FROM public\.garden_node WHERE archived_at IS NULL/,
    columns: ['created_at', 'archived_at'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/ux-events garden_node column contract', () => {
