// OPS-SCHEMAAUDITJOIN-001 — the public.garden_node columns lambda/projects reads.
//
// Four columns, but two of the seven statements name the table with NO alias — the deleted-
// planting count behind the two-step container recovery, and the active-planting count on project
// detail. Both are pinned below; an alias scan alone would report this directory as touching
// garden_node through `ga`/`gp` only and miss them.
//
// This is the relation BUG-SEEDDETAIL500-001 was on: `p.name` does not exist on garden_node (it is
// `display_name`), and it passed a green audit, a green unit suite and a green integration run
// while 500-ing every seed packet detail page in prod. lambda/inventory-items and lambda/plants
// already declare it; Phase 4 credits per directory, so neither buys these ten anything.
//
...
// than hardcoded, so a handler added here that JOINs garden_node is covered the day it lands
...
// L-081 KEYED contract. Every column below verified present on public.garden_node in live prod Neon on
...
const AUDIT_COLUMNS = {
  garden_node: ['archived_at', 'container_id', 'deleted_at', 'id'],
};
...
// group is OPTIONAL: an unaliased `FROM public.garden_node WHERE ...` captures the next keyword,
// which NOT_AN_ALIAS rejects and UNALIASED_ARMS then has to account for by hand.
const bindings = (s) => [...s.matchAll(
  /\b(?:FROM|JOIN)\s+(?:public\.)?garden_node\b(?!\s*\.)\s*(?:AS\s+)?([a-z_][a-z0-9_]*)?/gi,
...
// Scoped to statements that BIND garden_node, so an `x.col` belonging to some other query in the
...
// Reads that name garden_node with NO alias. Nothing can attribute their bare identifiers
...
const UNALIASED_ARMS = [
  {
    file: 'index.js',
    // deleted_planting_count. Plantings under a deleted container are invisible to
    // /api/plants/deleted until the container is restored, so this number is the only hint the user
    // gets that a second recovery step exists.
    pin: /SELECT container_id, count\(\*\)::int AS n\s+FROM public\.garden_node\s+WHERE container_id = ANY\(/,
    columns: ['container_id', 'deleted_at'],
  },
  {
    file: 'index.js',
    // The project-detail planting count. Note it is the one reference in this file written
    // WITHOUT the public. schema prefix — the auditor's relation regex makes that prefix optional and
    // so must this pin.
    pin: /SELECT COUNT\(\*\)::int AS count\s+FROM garden_node\s+WHERE container_id = \$\{projectId\}/,
    columns: ['container_id', 'deleted_at', 'archived_at'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/projects garden_node column contract', () => {
  it('finds the garden_node statements, so the assertions below are not vacuous', () => {
  ...
  it('accounts for every unaliased garden_node read', () => {
  ...
  it('references no column absent from the contract, and declares none it does not use', () => {
    ...
    expect(referenced).toEqual([...GARDEN_NODE_COLUMNS].sort());
  });

  it('never reaches for a column that belongs to another table', () => {
    // `name` is the regression itself. The other three are the plausible confusions:
    // event_log carries project_id and plant_id, and inventory_items carries variety_id — none of
    // the four exist on garden_node.
    const NOT_ON_TABLE = ['name', 'project_id', 'plant_id', 'variety_id'];
    for (const col of NOT_ON_TABLE) {
      expect(GARDEN_NODE_COLUMNS).not.toContain(col);
      for (const { file, sql } of STATEMENTS) {
        for (const a of aliasesOf(sql)) {
          expect(sql, `${file}: ${a}.${col} is not a garden_node column`)
  ...
  it('exposes the contract in the shape scripts/dev-main-schema-audit.py can parse', () => {
    ...
    expect(pairs.map((m) => m[1])).toEqual(['garden_node']);
    const cols = [...pairs[0][2].matchAll(/['"]([a-zA-Z_][a-zA-Z0-9_]*)['"]/g)].map((m) => m[1]);
    expect(cols).toEqual(GARDEN_NODE_COLUMNS);
  });
});
