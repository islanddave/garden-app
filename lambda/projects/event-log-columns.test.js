// OPS-SCHEMAAUDITJOIN-001 — the public.event_log columns lambda/projects reads.
//
// The five columns here come from authz-parents.js, the write-FK parent resolver that is
// byte-identical in seven directories (one sha256) for the same reason household.js is: each Lambda
// is zipped from its OWN directory, so a `../authz-parents.js` import is not packaged and the
// handler 502s at module load. Phase 4 groups by Path(handler).parent, so the copy in this
// directory needs its own contract — a contract beside any of the other six buys this one nothing.
//
// index.js adds a fifth statement written with NO alias: the public project event list, and the
// only read in this directory that touches is_public, notes and quantity. Its is_public filter is
// deny-by-default — that read is what keeps private event content out of a public project page —
// so the pin below is a privacy guard as much as a column contract.
//
...
const AUDIT_COLUMNS = {
  event_log: [
    'created_by', 'deleted_at', 'event_date', 'event_type', 'id', 'is_public', 'notes',
    'plant_id', 'project_id', 'quantity'
  ],
};
...
const UNALIASED_ARMS = [
  {
    file: 'index.js',
    // The public project event list. AND is_public IS TRUE is deny-by-default: drop it and
    // private notes reach an unauthenticated page. Pinned in full for that reason.
    pin: /SELECT id, event_type, event_date, notes, quantity\s+FROM event_log\s+WHERE project_id = \$\{row\.id\}\s+AND is_public IS TRUE\s+AND deleted_at IS NULL/,
    columns: [
      'id', 'event_type', 'event_date', 'notes', 'quantity', 'project_id', 'is_public',
      'deleted_at'
    ],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/projects event_log column contract', () => {
