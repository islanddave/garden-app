// OPS-SCHEMAAUDITJOIN-001 — the public.container columns lambda/dashboard reads.
//
// Fourteen statements through one alias, plus three that name the table with no alias:
// the project-count scalar sub-select, the dismiss-inactive CTE (the only reader of status here)
// and project search, which is the only reader of description and species anywhere in this
// directory. All three are pinned below.
//
...
const AUDIT_COLUMNS = {
  container: [
    'archived_at', 'assignee_user_id', 'created_at', 'created_by', 'deleted_at', 'description',
    'display_name', 'id', 'species', 'start_date', 'status', 'variety'
  ],
};
...
const UNALIASED_ARMS = [
  {
    file: 'handlers.js',
    // The project_count tile. Counts alive, unarchived containers for the household —
    // archived_at and deleted_at are separate axes here and both filters are load-bearing.
    pin: /FROM public\.container\s+WHERE created_by = ANY\(\$\{householdIds\}\) AND deleted_at IS NULL AND archived_at IS NULL\s+\) AS project_count/,
    columns: ['created_by', 'deleted_at', 'archived_at'],
  },
  {
    file: 'handlers.js',
    // queryDismissInactive's ownership CTE. Its status IN ('harvested','ended') is what
    // stops a live project being dismissed; this is the only read of status in this directory.
    pin: /SELECT id FROM public\.container\s+WHERE id = \$\{projectId\}::uuid\s+AND created_by = ANY\(\$\{householdIds\}\)\s+AND status IN \('harvested','ended'\)/,
    columns: ['id', 'created_by', 'status', 'deleted_at', 'archived_at'],
  },
  {
    file: 'handlers.js',
    // searchProjects. The sole reader of description and species in this directory — both
    // are matched with ILIKE and one of them is truncated into the result snippet, so a rename
    // would quietly return fewer search hits rather than error.
    pin: /LEFT\(COALESCE\(description, ''\), 160\) AS snippet\s+FROM public\.container/,
    columns: [
      'id', 'display_name', 'status', 'species', 'variety', 'description', 'created_by',
      'deleted_at', 'archived_at'
    ],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/dashboard container column contract', () => {
