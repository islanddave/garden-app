...
const UNALIASED_ARMS = [
  {
    file: 'critterAward.js',
    // V3-ARCHIVE-001's reward-suppression check. Fails OPEN on error, so a column typo here
    // would silently award critters on archived plantings rather than 500 — no signal at runtime.
    pin: /SELECT\s+archived_at\s+FROM public\.garden_node\s+WHERE id = \$\{plantId\}/,
    columns: ['archived_at', 'id'],
  },
  {
    file: 'index.js',
    // The give-away availability read. Its inner sub-select scans event_log through the `el`
    // alias, so the bare identifiers in this arm are NOT all garden_node's — they are listed here by
    // hand rather than scraped.
    pin: /COALESCE\(qty_harvested, 0\)[\s\S]*?FROM public\.garden_node\s+WHERE id =/,
    columns: ['quantity', 'qty_harvested', 'qty_lost', 'id', 'deleted_at'],
  },
];

describe('OPS-SCHEMAAUDITJOIN-001 — lambda/events garden_node column contract', () => {
