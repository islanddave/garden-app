// OPS-L081COLS-001 — static SELECT-column contract for events.
//
...
//
// events is the core write path — 3,074 lines in index.js alone, 13 files, and the single highest-value
// enrolment in the codebase. Every harvest, watering and observation the garden records passes through
// event_log, so an L-081 divergence here 500s essentially the whole app.
//
// The list below is EVIDENCE-DERIVED: the intersection of event_log's live prod columns with the
...
describe('events SELECT-column contract (L-081 Phase 1)', () => {
  ...
  it('pins a non-trivial contract', () => {
    expect(AUDITED_COLUMNS.length).toBeGreaterThanOrEqual(26);
  });
});
