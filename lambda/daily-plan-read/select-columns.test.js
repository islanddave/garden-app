// OPS-L081COLS-001 — static SELECT-column contract for daily-plan-read.
//
...
//
// daily-plan-read serves the plan the nightly job wrote; a column drift here breaks Today for every user.
//
// The list below is EVIDENCE-DERIVED: the intersection of daily_plan's live prod columns with the
...
describe('daily-plan-read SELECT-column contract (L-081 Phase 1)', () => {
  it('declares exactly one audit table, so the auditor cross-product stays honest', () => {
    expect(AUDIT_TABLES).toEqual(['daily_plan']);
  });
