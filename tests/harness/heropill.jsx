...
// The bar names which rung-2 rule the served keyFact.js carries, read off the function itself, so a
// capture says what code it shows rather than trusting the command line.
...
import { selectKeyFact } from '../../src/lib/keyFact.js'
...
const RULE = (() => {
  ...
  if (/crop_type_slug\s*===\s*["']tomato["']/.test(s)) return 'keyFact: crop type + determinacyLabel (branch head)'
  if (s.includes('determinacyLabel')) return 'keyFact: names + determinacyLabel (commit A)'
  return 'keyFact: names + prose fallback (before the fix)'
})()
...
window.__h = {
  ...
  census() {
    ...
    return {
      innerWidth, innerHeight, dpr: devicePixelRatio, rule: RULE, h1: h1?.textContent ?? null,
...
const paint = () => {
  ...
  el.textContent = firstError ? 'ERROR: ' + firstError
    : `${CASE} · innerWidth ${c.innerWidth} x ${c.innerHeight} @${c.dpr} · ${c.rule}\n`
