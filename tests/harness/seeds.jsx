...
// line wraps under a 44-character name with two chips, or how many rows are on the first screen.
...
//   · 27 seed rows over 8 crops, pepper-dominant then tomato, with a tail — the prod distribution's
//     shape (pepper 95 and tomato 40 of 263, measured for V5-SEEDSAVEDFILTER-001), scaled down.
...
//     and ARCHIVED FOR THIS SEASON, so its second line carries two chips ("Fermenting · day 5",
//     "Archived for this season") before its facts, and the ferment line under the switch names it.
//     Day 5 is FERMENT_ALARM_DAYS, so the line reads "overdue" — its longest wording.
//   · Bought packets with REGISTRY vendors (source_id -> /api/varieties/sources), and a `source`
//     column holding an ORDER REFERENCE, which is what prod keeps there — the page must print the
//     vendor, never the order text.
//   · A used-up packet (quantity 0), which My seeds files under the collapsed "Sowed previously".
//   · An identical pair (same cultivar, vendor, year and count), so labelCandidates' ordinal suffix
//     ("1 of 2 with identical details") is on screen and its width is measured.
...
//     a stored saved lot with a counted yield.
//
...
const SOURCES = [
  ...
  { id: 'src-baker', name: 'Baker Creek Heirloom Seeds' },
  { id: 'src-johnny', name: "Johnny's Selected Seeds" },
  { id: 'src-botanical', name: 'Botanical Interests' },
  { id: 'src-refire', name: 'Refining Fire Chiles' },
]
const CROP_TYPES = [
...
// One row of GET /api/inventory-items?category=seeds: `i.*` plus the list query's three projections
// (variety_name, crop_slug, stage_entered_at — lambda/inventory-items/index.js). Every column the
// Seeds views read is present, nulled when unset, because an ABSENT key and a NULL one are different
// facts to the sow engine (isUnstartedSave's un-widened-view guard reads hasOwnProperty).
let n = 0
...
const seed = (variety, crop, over = {}) => ({
  ...
  source_plant_id: null, source_kind: null, source_id: null, source: null, purchase_date: null,
...
const ROWS = [
  // pepper — the dominant crop, and the identical pair.
  bought('Serrano', 'pepper', 'src-fedco', '2026-01-14'),
  bought('Serrano', 'pepper', 'src-fedco', '2026-01-14'),
  bought('Hot Portugal', 'pepper', 'src-baker', '2025-12-30', { quantity_on_hand: 2 }),
  bought('Biquinho Red & Yellow Blend', 'pepper', 'src-baker', '2026-02-11'),
  bought('Megatron F1 (jumbo jalapeno)', 'pepper', 'src-johnny', '2026-02-11'),
  bought('Carolina Reaper', 'pepper', 'src-refire', '2025-03-02', { unit: 'each', quantity_on_hand: 25 }),
  bought('Fish', 'pepper', 'src-baker', '2024-01-20'),
  bought('Shishito', 'pepper', 'src-botanical', '2026-01-14', { quantity_on_hand: 3 }),
  seed('Lemon Drop', 'pepper'),                                            // no vendor, no date on record
  seed('Aji Charapita', 'pepper', {                                         // a saved lot, drying
  ...
  bought('Sungold F1', 'tomato', 'src-johnny', '2026-01-14'),
  bought("Brandywine (Sudduth's Strain)", 'tomato', 'src-baker', '2025-01-05'),
  bought('Green Zebra', 'tomato', 'src-fedco', '2026-01-14'),
  bought('Amish Paste', 'tomato', 'src-fedco', '2024-12-01', { quantity_on_hand: 2 }),
  bought('Sunrise Bumble Bee', 'tomato', 'src-baker', '2026-02-11', { status: 'retired' }),
  // lettuce — including the used-up packet.
  bought('Black Seeded Simpson', 'lettuce', 'src-botanical', '2026-06-09'),
  bought('Winter Density', 'lettuce', 'src-fedco', '2026-01-14'),
  ...
  // the tail
  bought("Cinderella (Rouge Vif d'Etampes)", 'winter_squash', 'src-baker', '2025-12-30'),
  ...
  bought('Pennsylvania Dutch Crookneck', 'summer_squash', 'src-baker', '2025-12-30'),
...
window.fetch = (url, ...rest) => {
  const u = String(url)
  const hit = (k, body) => { hits[k] = (hits[k] ?? 0) + 1; return json(body) }
...
const ARRIVED = {
  mine: () => document.querySelectorAll('[data-testid="my-seed-row"]').length > 0,
...
async function run() {
  ...
  window.__h = {
    ...
    errors: () => [...errors],
    hits: () => ({ ...hits }),
    fixture: () => ({ rows: ROWS.length, sowCandidates: SOW_CANDIDATES.length, longName: LONG_NAME, year: YEAR }),
    all: measure,
  }
  paint()
}
...
function paint() {
  ...
  el.textContent = [
    ...
    `switch one line=${m.switchOneLine}  rows=${m.rows}  cards=${m.cards}  ferment line=${m.ferment}`,
  ].join('\n')
