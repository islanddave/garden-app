...
# getComputedStyle / Range.getClientRects in a real Chrome at the viewport the capture self-reports
# (426x836 @ DPR 3 by default since 2026-09-24 — Dave's handset; drive.mjs).
import json, sys, collections, re

# Taken from the capture in main(), never assumed: a capture at another width must not be divided
# by this one's height.
VW, VH = None, None
...
def main(path):
    global VW, VH
    d = load(path)
    VW, VH = d['viewport']['innerWidth'], d['viewport']['innerHeight']
    ink = d['ink']['ink']
