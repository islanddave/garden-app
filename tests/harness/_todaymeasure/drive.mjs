#!/usr/bin/env node
// drive.mjs — drive the real Today page in real Chrome at a TRUE 426x836 @ DPR 3 (Dave's handset,
// read 2026-09-24) and record what it does.
//
...
// proceed on is an unproven instrument — if the page does not self-report the requested innerWidth, or if it
...
// floors an OS window at ~500px, so a narrower --window-size lays out at 500 and crops the capture —
// a plausible-looking mobile screenshot of a desktop-width layout. (Trap documented at length
...
// 5324 / 9434 — clear of every layout gate's defaults (see scripts/layout-gate/today-shape.mjs).
const PORT = Number(process.env.TM_PORT || 5324)
const CDP_PORT = Number(process.env.TM_CDP_PORT || 9434)
const CHROME = process.env.CHROME_PATH || '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'
// Same env names and defaults as the gate, so the recorder and the gate describe one viewport.
const VIEWPORT = {
...
try {
  ...
  for (const state of STATES) {
    ...
    writeFileSync(join(OUT, `today-${state}-${VIEWPORT.w}-full.png`), Buffer.from(full.data, 'base64'))
    ...
    for (let s = 0; s < slices; s++) {
      ...
      writeFileSync(join(OUT, `today-${state}-${VIEWPORT.w}-screen${String(s + 1).padStart(2, '0')}.png`), Buffer.from(shot.data, 'base64'))
    }
    ...
    writeFileSync(join(OUT, `today-${state}-${VIEWPORT.w}-badge.png`), Buffer.from(badged.data, 'base64'))
