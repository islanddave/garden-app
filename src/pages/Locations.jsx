...
function LocRow({ loc, indent, open, hasKids, onToggleExpand, onToggleActive }) {
  ...
  return (
    <div style={{
      ...
      {/* Line 1 — expand + name + type badge + inactive badge */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minHeight: 32 }}>
        ...
        <span style={{ fontWeight: isZone ? 700 : 500, color: P.dark, fontSize: isZone ? '0.95rem' : '0.88rem', wordBreak: 'break-word', flex: 1 }}>
          {loc.name}
        </span>
        {loc.type_label && (
          <span style={{ fontSize: '0.7rem', color: P.mid, backgroundColor: P.border, borderRadius: 10, padding: '1px 7px', flexShrink: 0 }}>
            {loc.type_label}
          </span>
        )}
        {!loc.is_active && (
          <span style={{ fontSize: '0.7rem', color: P.light, backgroundColor: '#eee', borderRadius: 10, padding: '1px 7px', flexShrink: 0 }}>
            inactive
          </span>
        )}
      </div>
      {/* Line 2 — slug · level label + deactivate button */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 5, paddingLeft: 24 }}>
        <span style={{ fontSize: '0.72rem', color: P.light, fontFamily: 'monospace' }}>
          /{loc.slug} · {LEVEL_LABELS[loc.level]}
        </span>
        <button
          onClick={() => onToggleActive(loc)}
          style={{
            fontSize: '0.72rem',
            color: loc.is_active ? P.terra : P.green,
            background: 'none',
            border: `1px solid ${loc.is_active ? P.alertBorder : P.greenLight}`,
            borderRadius: 4,
            padding: '4px 10px',
            cursor: 'pointer',
            flexShrink: 0,
