  ...
  return (
    <div style={{ border: `1px solid ${P.border}`, borderRadius: 8, overflow: 'hidden' }}>
      <LocRow loc={zone} indent={0} open={open} hasKids={directChildren.length > 0}
        onToggleExpand={() => setOpen(o => !o)} {...editProps} />
      {open && directChildren.map(child => {
        const grandkids = locations.filter(l => l.parent_id === child.id)
        return (
          <div key={child.id}>
            <LocRow loc={child} indent={1} hasKids={false} {...editProps} />
            {grandkids.map(gc => {
              const positions = locations.filter(l => l.parent_id === gc.id)
              return (
                <div key={gc.id}>
                  <LocRow loc={gc} indent={2} hasKids={false} {...editProps} />
                  {positions.map(pos =>
                    <LocRow key={pos.id} loc={pos} indent={3} hasKids={false} {...editProps} />
                  )}
                </div>
              )
            })}
