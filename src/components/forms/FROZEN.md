...
| **FacetGroupHeader** | Section header for one group in the faceted Garden render: facet icon + value + count, collapsible-disclosure per the Garden accordion. | Shipped. Signature is a superset of the contract's `({facet, value, count})` — compatible. V5-SEEDCARDS-001 draws the count at full opacity (was 0.7, which read 3.74:1 on its fill) — a visual change on EVERY caller, Garden's group headers included; My seeds also uses the collapsible mode. |
