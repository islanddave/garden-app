...
// twice over:
//  1. The live dataset contains exactly one record (sweet_potato, check 10-01, deadline 10-15) and
...
// The real dataset's sweet_potato record: check window opens 10-01, deadline 10-15.
const sweet = (over = {}) => ({
...
describe('storageDeadlineGroups — the time bounds are the feature', () => {
  it('is SILENT today (2026-08-14): the check window has not opened', () => {
    ...
    // dataset opens on 10-01. Anything that makes this test go non-empty has widened the criteria.
    expect(storageDeadlineGroups([sweet()], '2026-08-14')).toEqual([])
  })

  it('is silent right up to the day before the check window opens', () => {
    expect(storageDeadlineGroups([sweet()], '2026-09-30')).toEqual([])
  })

  it('speaks on the day the check window opens, in CHECK form', () => {
    const g = storageDeadlineGroups([sweet()], '2026-10-01')
  ...
  it('still speaks on the deadline day itself', () => {
    expect(storageDeadlineGroups([sweet()], '2026-10-15')[0].phase).toBe('check')
  })

  it('switches to past copy the day after the deadline', () => {
    const g = storageDeadlineGroups([sweet()], '2026-10-16')
  ...
  it('keeps saying so through the grace window, then goes SILENT', () => {
    ...
    // 10-15 + 14 = 10-29, the last day inside the grace window.
    expect(storageDeadlineGroups([sweet()], '2026-10-29')).toHaveLength(1)
    ...
    expect(storageDeadlineGroups([sweet()], '2026-10-30')).toEqual([])
  })
...
describe('storageDeadlineGroups — what it will and will not speak about', () => {
  ...
  it('drops ended/failed plantings — never tell him to lift something he closed out', () => {
    expect(storageDeadlineGroups([sweet({ status: 'ended' })], '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups([sweet({ status: 'failed' })], '2026-10-02')).toEqual([])
  })

  it("does NOT drop status 'harvested' — that DB value is labelled 'Harvesting' (in progress)", () => {
    expect(storageDeadlineGroups([sweet({ status: 'harvested' })], '2026-10-02')).toHaveLength(1)
  })

  it('says nothing about a crop deliberately given no deadline (carrot)', () => {
    expect(storageDeadlineGroups([carrot()], '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups([carrot()], '2026-11-15')).toEqual([])
  })

  it('says nothing about a planting with no variety or no crop type', () => {
    expect(storageDeadlineGroups([{ id: 'x', name: 'Mystery', variety_ref: null }], '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups([{ id: 'x', name: 'M', variety_ref: { crop_type_slug: null } }], '2026-10-02')).toEqual([])
  })

  it('caps the group count and never throws on junk input', () => {
    expect(STORAGE_DEADLINE_CAP).toBe(2)
    expect(storageDeadlineGroups([sweet()], '2026-10-02', 0)).toEqual([])
    expect(storageDeadlineGroups([], '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups(null, '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups(undefined, '2026-10-02')).toEqual([])
    expect(storageDeadlineGroups([null, undefined, 7], '2026-10-02')).toEqual([])
...
describe('StorageDeadlineAlert component', () => {
  ...
  it('renders the sourced copy and the planting names once the window is open', async () => {
    fetchMock.mockResolvedValue([sweet(), sweet({ id: 'p-sp2', name: 'Georgia Jet' }), carrot()])
    render(<StorageDeadlineAlert todayISO="2026-10-02" />)
  ...
  it('swallows a fetch error — renders nothing, never throws onto Today', async () => {
    ...
    const { container } = render(<StorageDeadlineAlert todayISO="2026-10-02" />)
  ...
  it('renders nothing before the first load resolves, and nothing on an empty garden', async () => {
    ...
    const second = render(<StorageDeadlineAlert todayISO="2026-10-02" />)
