...
describe('ProjectDetail — V3-FORM-001 / V3-FORM-002 (launch-point parity)', () => {
  ...
  it('FORM-001: POST body carries container_type/container_size when set on add', async () => {
    wireApiFetch()
    await act(async () => { render(<ProjectDetail />) })
    await openAddPlantForm()
    const nameInput = screen.getByPlaceholderText(/Megatron Jalapeno/)
    await act(async () => { fireEvent.change(nameInput, { target: { value: 'Potted Plant' } }) })
    const ctype = document.getElementById('add-plant-ctype')
    const csize = document.getElementById('add-plant-csize')
    expect(ctype).toBeTruthy()
    await act(async () => { fireEvent.change(ctype, { target: { value: 'fabric_bag' } }) })
    await act(async () => { fireEvent.change(csize, { target: { value: '5 gal' } }) })
    const form = nameInput.closest('form')
    await act(async () => { fireEvent.submit(form) })
    await waitFor(() => expect(postCalls.length).toBeGreaterThan(0))
    expect(postCalls[0]).toMatchObject({ container_type: 'fabric_bag', container_size: '5 gal' })
  })
})
