...
describe('EventNew — observation flag form', () => {
  ...
  it('renders the flag toggle for observation events only', async () => {
    await setupObservation()
    expect(screen.getByLabelText('Flag as an issue')).toBeTruthy()
  })

  it('does NOT render the flag toggle for non-observation events', async () => {
    renderEventNew('event_type=watering')
    await flushLoad()
    expect(screen.queryByLabelText('Flag as an issue')).toBeNull()
  })

  it('shows the severity select only when the flag toggle is on', async () => {
    await setupObservation()
    expect(screen.queryByLabelText('Severity')).toBeNull()
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    expect(screen.getByLabelText('Severity')).toBeTruthy()
  })

  it('toggling the flag off clears the severity state', async () => {
    await setupObservation()
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    fireEvent.change(screen.getByLabelText('Severity'), { target: { value: '3' } })
    expect(screen.getByLabelText('Severity').value).toBe('3')
    // toggle off
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    expect(screen.queryByLabelText('Severity')).toBeNull()
    // toggle back on — severity should be reset, not '3'
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    expect(screen.getByLabelText('Severity').value).toBe('')
  })

  it('severity dropdown does NOT include a "Stale" option', async () => {
    await setupObservation()
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    const options = Array.from(screen.getByLabelText('Severity').querySelectorAll('option'))
    expect(options.some(o => /stale/i.test(o.textContent))).toBe(false)
  })

  it('submitting a flagged observation with no severity shows inline error and does NOT POST', async () => {
    await setupObservation()
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    await act(async () => {
      fireEvent.click(screen.getByText('+ Log event'))
    ...
    expect(postCalls.length).toBe(0)
    expect(screen.getByText('Pick a severity to flag.')).toBeTruthy()
  })

  it('setting severity then submitting POSTs flagged_as_issue:true + severity', async () => {
    await setupObservation()
    fireEvent.click(screen.getByLabelText('Flag as an issue'))
    ...
    expect(postCalls.length).toBe(1)
    expect(postCalls[0].flagged_as_issue).toBe(true)
    expect(postCalls[0].severity).toBe(2)
  })

  it('an unflagged observation submit sends neither flagged_as_issue nor severity', async () => {
    await setupObservation()
    await act(async () => {
      fireEvent.click(screen.getByText('+ Log event'))
    })
    ...
    expect(postCalls[0].severity).toBeUndefined()
  })
})
