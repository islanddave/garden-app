...
//
//   • re-bundled  — someone "simplifies" App.jsx's `loadCollectionChunk()` (src/lib/collectionChunk.js,
//                   which holds the `import('../pages/Collection.jsx')`) back to a static
//                   `import Collection from './pages/Collection.jsx'`. The feature still works, every
//                   existing test stays green, and ~40KB gzip silently returns to the Android boot
//                   path. Nothing red. This is the inert-regression class.
//   • inert       — the boundary survives but the loader never resolves (bad path, dropped export),
//                   so the route sits on the fallback forever. A source-text grep for `import(` passes.
//
...
//   (b) rendering /collection must show the loading fallback FIRST (the chunk is fetched in an
//       effect — a static import renders synchronously and never does), and then must actually reach
//       the module and render what it exports.
//
// This file pins that the chunk IS split and IS reached. It says nothing about what happens when the
// fetch FAILS — that is App.collectionChunkRetry.test.jsx (V4-LAZYRETRY-001), and the two are a pair:
// a change that keeps the split but reintroduces React.lazy passes here and reds there.
//
...
// RE-PROVEN 2026-08-26 after V4-LAZYRETRY-001 reshaped the boundary (React.lazy → collectionChunk.js):
//   • inert      — loadCollectionChunk stubbed to `Promise.resolve(() => null)`, so the wrapper and
//     the split both survive: the two shape assertions PASS and only 'rendering /collection shows the
//     loading state first, then reaches the chunk' goes RED (1 failed | 2 passed). Unchanged verdict.
...
import { renderRoutes, CollectionRoute } from '../App.jsx'
...
describe('V4-COLLECTIONSPLIT-001 — /collection is a real code-split route', () => {
  ...
  it('the route element defers to the async chunk wrapper, not to a plain page component', () => {
    // Structural, not source-text, and checked THROUGH the route table so a wrapper that never made
    // it onto the route cannot pass. A static `import Collection from …` puts the page component
    // here directly and fails this immediately (and test (a) with it).
    //
    // V4-LAZYRETRY-001 changed the shape this asserts. It used to be
    // <React.Suspense><LazyCollection/></React.Suspense> and checked for Symbol.for('react.lazy').
    // React.lazy is now BANNED here, not required: it caches a rejected payload permanently, so a
    // chunk that failed once could never be retried. The boundary is a plain import() inside
    // CollectionRoute instead — see src/lib/collectionChunk.js — and the retry behaviour that
    // replaces it is gated by App.collectionChunkRetry.test.jsx.
    const boundary = collectionRoute().props.element.props.children
    expect(boundary.type).toBe(ErrorBoundary)
    expect(boundary.props.scope).toBe('route')
    expect(boundary.props.children.type).toBe(CollectionRoute)
  })

  it('rendering /collection shows the loading state first, then reaches the chunk', async () => {
    render(
    ...
    // Deferred: the wrapper is showing its loading state and the page is NOT in the tree yet. A
    // static import renders synchronously, so this pair is what makes the guard non-vacuous.
