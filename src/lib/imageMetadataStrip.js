...
//
// THIS FILE IS THE CANONICAL COPY AND IT IS MIRRORED INTO A LAMBDA — BUG-FBSHAREDENYLIST-001.
// lambda/facebook-share/ used to carry its own narrower DENYLIST strip (drop APP1, copy everything
// from SOS to EOF verbatim), left alone as "defence-in-depth on a path this module does not reach".
// That was backwards: Facebook share is the ONE exit that publishes a photo publicly, outside the
// household, so the weakest strip in the codebase was guarding the highest-exposure path. Measured
// on a phone-shaped fixture (412 B), the denylist version passed 322 B through with the Samsung
// MCC_Data/DCIM trailer, the appended Pixel gain-map JPEG, APP11 JUMBF, APP13 IPTC and a COM
// comment all intact — because it never looked past EOI and dropped only the one marker it named.
// lambda/facebook-share/imageMetadataStrip.js is now a BYTE-IDENTICAL copy of this file, pinned by
// lambda/imageMetadataStrip-copies-sync.test.js. Copied rather than imported because deploy-lambda.yml
// zips from inside the function dir (`cd lambda/<fn> && zip -r ../<fn>.zip .`), so a `../../src/lib`
// import resolves under vitest and is ERR_MODULE_NOT_FOUND in the deployed Lambda — same constraint
// and same house pattern as lambda/http-response.js and lambda/photo-access.js.
// EDIT THIS FILE, THEN RE-COPY. Editing the copy alone reds the sync test, which is the point.
//
// MERGE NOTE (integration-fleet4, 2026-08-21): the `heicreal` lane branched from dev before the
// `fbstrip` lane landed, so its version of this header still said "lambda/facebook-share/exif.js is
// a separate, narrower denylist strip ... left alone on purpose." That sentence was true when it was
// written and is now FALSE — fbstrip replaced that denylist, which had been passing 322 of 412
// fixture bytes. Dropped rather than merged, so nobody reinstates a denylist on the one path that
