...
--
-- To retract the BACKFILL without dropping the table (the usual case — the schema is fine, the
-- derived values are not wanted), run this instead and stop:
--
--   DELETE FROM public.plant_anchor_derivation WHERE model_version = 'anchor-derive-v1';
--
-- The table is standalone. Its single outbound FK is to public.plants and nothing references it, so
-- the drop cannot cascade into user data.
