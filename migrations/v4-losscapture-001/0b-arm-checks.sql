...
VALUES ('4.25.1-losscapture-001-checks','V4-LOSSEVENT-001/BUG-LOSSCAUSE-001/V4-HARVDISPOSITION-001 phase 2/3 (DEPLOY-ORDERED). DROPs the legacy hand-applied plants_loss_cause_check (5 values, VALIDATED) and consolidates it into chk_plants_loss_cause WIDENED TO SEVEN (pest|disease|weather|transplant_shock|unknown|animal_damage|culled); also arms chk_plants_qty_lost_nonneg (>=0) and chk_harvest_log_disposition (dropped|culled|aborted|damaged). All three NOT VALID, so existing rows are exempt and every subsequent write is constrained. Ordering runs in BOTH directions: qty_lost narrowing is CODE FIRST (gate pre_qty_lost_guard_is_deployed — lambda/plants validateQtyLost must already be LIVE on both verbs, and was, as of prod 055ba97), while the loss_cause widening is SCHEMA FIRST — this file precedes the plants Lambda carrying the seven-value ALLOWED_LOSS, which is NOT yet deployed at apply time. VALIDATE deferred to 0c-validate.sql.')
